...
import { Plus, Loader2, Building2, MapPin, Maximize, DollarSign, AlertCircle } from "lucide-react";
import { imovelService } from "@/services/api";
import { ImovelRequest } from "@/types";
...
const INITIAL_FORM_DATA: ImovelRequest = {
    endereco: "",
    valor: 0,
    metrosQuadrados: 0,
    disponivel: true,
...
export function NovoImovelModal({ onSucesso }: Props) {
    const [open, setOpen] = useState(false);
    const [loading, setLoading] = useState(false);
    const [formData, setFormData] = useState<ImovelRequest>(INITIAL_FORM_DATA);
    ...
    const validateForm = (): boolean => {
        ...
        // Valor deve ser inteiro maior que 0
        if (!formData.valor || formData.valor <= 0) {
            newErrors.valor = "Valor deve ser maior que 0";
        } else if (!Number.isInteger(formData.valor)) {
            newErrors.valor = "Valor deve ser um número inteiro (sem centavos)";
        }

        // Área deve ser inteiro maior que 0
    ...
    // Função para lidar com input numérico (apenas inteiros)
                ...
                <form onSubmit={handleSubmit} className="space-y-5 pt-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-muted-foreground">Apenas números inteiros</p>
                        </div>
